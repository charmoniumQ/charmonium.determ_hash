...
def _determ_hash(obj: Any, hasher: Hasher, verbose: bool, level: int) -> None:
    if verbose:
    ...
    if isinstance(obj, type(None)):
        ...
        _determ_hash(obj.imag, hasher, verbose, level + 1)
        _determ_hash(obj.real, hasher, verbose, level + 1)
    elif isinstance(obj, type(...)):
        hasher.update(b"...")
    elif isinstance(obj, tuple):
        hasher.update(b"tuple(")
        for elem in cast(Tuple[Hashable], obj):
            _determ_hash(elem, hasher, verbose, level)
        hasher.update(b")")
    elif isinstance(obj, CodeType):
        hasher.update(b"code(")
        hasher.update(obj.co_code)
        _determ_hash(obj.co_consts, hasher, verbose, level + 1)
        hasher.update(b")")
        ...
        qualname: str = obj.__qualname__  # type: ignore
        ...
        _determ_hash(proxy, hasher, verbose, level)
    else:
...
def getclosurevars(func: FunctionType) -> inspect.ClosureVars:
